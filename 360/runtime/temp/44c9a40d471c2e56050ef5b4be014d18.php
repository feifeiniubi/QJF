<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:98:"/Applications/XAMPP/xamppfiles/htdocs/360/public_html/../application/member/view/login/index.phtml";i:1505287915;}*/ ?>
<!doctype html>
<html lang="zh-CN">
<head>
    <meta charset="UTF-8">
                    ...
                    <form method="post" id="loginForm">
                        <fieldset>
                            <div class="form-group">
                                <label>用户名：</label>
                                <input type="text" class="form-control" placeholder="请输入用户名或者手机号码" name="username" id="username">
                            </div>
                            <div class="form-group">
                                <label>密码：</label>
                        ...
                        <!--a href="/member/login/register/" class="btn btn-block">注册账号</a-->
                    </form>
                </div>
            </div>
        </div>
    ...
    $(function () {
        $('#loginForm').myForm(function (ret) {
            ...
            if (isCkd) {
                ...
                window.localStorage.setItem('memberLogin', JSON.stringify(memberLogin))
            }
            // 跳转
            window.location.href = ret.url;
        });
        // 自动填空
        var memberLogin = window.localStorage.getItem('memberLogin');
        if (memberLogin) {
            memberLogin = JSON.parse(memberLogin);
            $('#username').val(memberLogin.username);
            $('#password').val(memberLogin.password);
            $('#remember').prop('checked', true);
//            $('#loginForm').submit();
        }
        // 清除记录
        $('#remember').on('click', function () {
            window.localStorage.setItem('memberLogin', '');
        });
